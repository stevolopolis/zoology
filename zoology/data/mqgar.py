...
class MQGARConfig(DataSegmentConfig):
    name: str="multiquery_ar_surjective"
    power_a: float=0.01
    num_kv_pairs: int=8
    random_non_queries: bool=True
    include_slices: bool=True
    surjective_ratio: float=1.0
    ...
    def build(self, seed: int) -> DataSegment:
        return multiquery_general_ar(**self.model_dump(), seed=seed)

def multiquery_general_ar(
    vocab_size: int,
    num_examples: int,
    input_seq_len: int,
    seed: int,
    ...
    **kwargs
    ...
    Args:
        ...
        surjective_ratio (float, optional): The ratio of values to replace with the other half. Defaults to 0.5.
        query_max_len (int, optional): The maximum number of tokens per query. Defaults to 1.
        share_keys (int, optional): The number of keys, starting from the last key, to share. Defaults to 0.
    Returns:
    ...
    """
    assert input_seq_len % (1 + query_max_len) == 0, "input_seq_len must be divisible by (1 + query_max_len)"
    assert vocab_size > input_seq_len, "Vocabulary size must be greater than input sequence length" 
    assert num_kv_pairs * (1 + query_max_len) * 2 <= input_seq_len, "Input sequence length is too short for the given number of key-value pairs and query max length"
    assert share_keys < query_max_len, "Share keys must be less than query max length"
    assert not (min_keys and share_keys > 0), "Min keys and share keys cannot be used together"
    assert n_unique_values == 0 or surjective_ratio == 1.0, "N unique values and surjective ratio cannot be used together"

    np.random.seed(seed)

    # two tokens for key and value
    context_size = num_kv_pairs * (1 + query_max_len)
    ...
    # Calculate the minimum number of unique keys needed to generate the desired number of kv pairs
    min_unique_keys = max(4, math.ceil(num_kv_pairs ** (1 / query_max_len)))
    # Define a helper function to randomly generate a set of unique multi-token keys from a minimum subset of unique keys
    def generate_key_combinations(key_choices, query_max_len):
        # Extract a minimum set of unique keys
        key_subset = np.random.choice(key_choices, size=min_unique_keys, replace=False)
        # Generate the set of all combination of keys along
        multi_token_keys = np.array(list(itertools.product(key_subset, repeat=query_max_len)))
        select_idx = np.random.choice(len(multi_token_keys), replace=False, size=num_kv_pairs)

        return multi_token_keys[select_idx]

    keys_unshuffled = np.tile(key_choices, (num_examples, 1))
    if min_keys and query_max_len > 1:
        print("Using minimum unique keys")
        keys = np.apply_along_axis(generate_key_combinations, 1, keys_unshuffled, query_max_len=query_max_len)
    else:
        keys = np.apply_along_axis(np.random.choice, 1, keys_unshuffled, replace=False, size=(num_kv_pairs, query_max_len))

    # Set the last key token to be the same if share_last_key is True
    if share_keys > 0:
        print(f"Sharing last {share_keys} keys")
        keys[:, :, -share_keys:] = keys[:, 0, -share_keys:][:, np.newaxis, :]

    values_unshuffled = np.tile(value_choices, (num_examples, 1))
    n_unique_values = int(num_kv_pairs * surjective_ratio) if n_unique_values == 0 else n_unique_values
    values = np.apply_along_axis(np.random.choice, 1, values_unshuffled, replace=False, size=n_unique_values)

    # Repeat values until its length is equal or larger than num_kv_pairs
    values = np.tile(values, (1, num_kv_pairs // values.shape[1] + 1))
    # Truncate the values so the length of example is equal to num_kv_pairs
    values = values[:, :num_kv_pairs]
    # Shuffle it again so that the values are not repeated in the same order 
    rng = np.random.default_rng(seed)
    values = np.apply_along_axis(rng.permutation, 1, values)

    # create sequences
    kvs = np.zeros((num_examples, context_size), dtype=np.int64)
    for i in range(query_max_len):
        kvs[:, i::(1+query_max_len)] = keys[:, :, i]
    kvs[:, query_max_len::(1+query_max_len)] = values

    # compute power law
    space = (input_seq_len - context_size) // (1+query_max_len)
    p = power_a * np.arange(1, space + 1) ** (power_a-1)
    p = p / p.sum()
    ...
    for i in range(query_max_len):
        np.put_along_axis(queries, (gaps * (1+query_max_len)) + i, values=keys[:, :, i], axis=1)
    ...
    np.put_along_axis(labels, (gaps * (1+query_max_len)) + context_size + query_max_len, values=values, axis=1)
