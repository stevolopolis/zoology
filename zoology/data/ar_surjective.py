...
class MQARSurjectiveConfig(DataSegmentConfig):
    name: str="multiquery_ar_surjective"
    power_a: float=0.01
    num_kv_pairs: int=8
    random_non_queries: bool=True
    include_slices: bool=True
    surjective_ratio: float=0.5

    def build(self, seed: int) -> DataSegment:
        return multiquery_ar_surjective(**self.model_dump(), seed=seed)

def multiquery_ar_surjective(
    vocab_size: int,
    num_examples: int,
    input_seq_len: int,
    seed: int,
    ...
    **kwargs
    ...
    values = np.apply_along_axis(np.random.choice, 1, values_unshuffled, replace=False, size=int(num_kv_pairs * surjective_ratio))
